...
async def generate_with_retry(request: LLMRequest, max_retries: int = 3, request_id: str = "N/A") -> Dict[str, Any]:
    ...
    log_with_context(
        ...
        prompt_preview=truncate_for_log(request.prompt, 100),  # Shorter preview for internal logs
...
async def generate_response(request: LLMRequest):
    ...
    try:
        ...
        # Enhanced logging with prompt content
        log_with_context(
            logger, 'info',
            ...
            prompt_length=len(request.prompt),
            prompt_preview=truncate_for_log(request.prompt, LOG_PROMPT_MAX_LENGTH),
            prompt_hash=hash(request.prompt) % 10000,  # Simple hash for tracking
            parameters=request.parameters,
            full_prompt=request.prompt if LOG_FULL_CONTENT else None
        )
        ...
        # Enhanced success logging with response content
        log_with_context(
            logger, 'info',
            ...
            response_preview=truncate_for_log(result['response'], LOG_RESPONSE_MAX_LENGTH),
            response_hash=hash(result['response']) % 10000,
            # Performance metrics
            ...
            full_prompt=request.prompt if LOG_FULL_CONTENT else None,
            full_response=result['response'] if LOG_FULL_CONTENT else None
        )
        ...
        # Enhanced error logging with prompt context
        log_with_context(
            logger, 'error',
            ...
            total_time_seconds=round(total_time, 3),
            # Include prompt context for debugging
            prompt_tokens=token_count,
            prompt_length=len(request.prompt),
            prompt_preview=truncate_for_log(request.prompt, LOG_PROMPT_MAX_LENGTH),
            prompt_hash=hash(request.prompt) % 10000,
        ...
        # Enhanced unexpected error logging with full context
        log_with_context(
            logger, 'error',
